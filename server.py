...
# Conservative email matching. The previous version accidentally converted full
# Retell summaries into fake email addresses like:
# "theagentcalled...wednesday@1030ampacifictime.thecall..."
EMAIL_RE_ALL = re.compile(
    r"(?<![A-Z0-9._%+-])([A-Z0-9._%+-]{1,64}@[A-Z0-9.-]{1,253}\.[A-Z]{2,24})(?![A-Z0-9._%+-])",
    re.I,
)
...
def is_valid_real_email(email: str) -> bool:
    ...
    # Reject fake sentence-like emails created from summaries/transcripts.
    compact = email.replace(".", "").replace("_", "").replace("-", "")
    if any(bad in compact for bad in BAD_EMAIL_SUBSTRINGS):
        return False

    if len(local) > 64 or len(domain) > 253:
        return False

    # Normal emails usually do not have huge run-on local/domain parts.
    if len(local) > 40:
        return False
    ...
    # Basic shape check after cleanup.
    return bool(re.fullmatch(r"[a-z0-9._%+\-]+@[a-z0-9.\-]+\.[a-z]{2,24}", email))
...
def extract_all_emails(text: str) -> List[str]:
    ...
    found: List[str] = []
    for match in EMAIL_RE_ALL.findall(text):
...
def normalize_spoken_email(text: str) -> str:
    ...
    # Convert only email-like spoken phrases. Keep word boundaries/spaces so the
    # regex cannot glue a whole call summary into one fake email.
    t = re.sub(r"\s*(?:\\[at\\]|\bat\b)\s*", "@", t)
    t = re.sub(r"\s*(?:\\[dot\\]|\bdot\b)\s*", ".", t)

    t = t.replace("g-mail", "gmail")
    t = t.replace("g mail", "gmail")
    t = t.replace("gee mail", "gmail")

    # Add spaces around obvious sentence punctuation to keep candidates bounded.
    t = re.sub(r"[,;:!?()\[\]{}<>\"']", " ", t)
    return t
...
def find_matching_row(
    ...
    # 3. Phone fallback is dangerous for demo submissions because the same user
    # may test multiple times. It can overwrite a previous row. Only allow it for
    # outbound/legacy sheets.
    if not allow_phone_fallback:
        return None
...
async def demo_lead(request: Request):
    ...
    # Every form submission gets its own row and its own unique lead_id.
    # Do NOT upsert by phone for demo tests, because repeated tests from the same
    # phone must not overwrite earlier call outcomes.
    demo_lead_id = (
    ...
    row_num = append_row_by_headers(
        ...
        demo_hm,
        {
            ...
            "email_primary": email if is_valid_real_email(email) else "",
            "source": source,
    ...
    print("STEP 3: new demo row appended", row_num, demo_lead_id)
    ...
    print("STEP 4: creating Retell call")
    try:
        ...
        print("RETELL CALL CREATED:", retell_call_id)

    except Exception as e:
        # Do not throw a 500 to the website form. Log the issue and keep the lead.
        error_msg = str(e)
    ...
    if email and is_valid_real_email(email):
        try:
...
async def retell_post_call(request: Request):
    ...
    # Use a form-provided email first. Only extract from summary/transcript if it
    # looks like a real email. Never scan the entire payload dict as text; that is
    # how fake summary-sentence emails get created.
    captured_email = ""
    if is_valid_real_email(meta_email):
        captured_email = meta_email
    else:
        captured_email = (
    ...
    row_num = find_matching_row(
        ws,
        hm,
        call_id,
        lead_id,
        to_number,
        allow_phone_fallback=(flow_type != "demo"),
    )

    # For demo calls, never overwrite an old row just because the phone matches.
    # If Retell sends an unexpected second call/webhook with a new call_id, create
    # a new row so the old outcome stays intact.
    if row_num is None and flow_type == "demo":
        row_num = append_row_by_headers(
            ws,
            hm,
            {
                ...
                "email_primary": (
                    captured_email if is_valid_real_email(captured_email) else ""
                ),
        ...
        print("CREATED NEW ROW FOR UNMATCHED DEMO WEBHOOK:", row_num)
    ...
    if captured_email and is_valid_real_email(captured_email):
        updates = {}
    ...
    if not email_for_klaviyo and captured_email and is_valid_real_email(captured_email):
