...
def normalize_spoken_email(text: str) -> str:
    """Turn spoken email phrases into regex-readable emails.

    Examples:
    - "dre dev at g mail dot com" -> "dredev@gmail.com"
    - "t h e d r a y d e v at g-mail dot com" -> "thedraydev@gmail.com"
    """
    if not text:
        return ""
    ...
    t = t.replace("hotmail . com", "hotmail.com")
    t = t.replace("outlook . com", "outlook.com")
    t = re.sub(r"\s+", "", t)
    return t


def pick_best_email_any_format(*texts: str) -> str:
    """Extract an email from normal text or spoken-email text."""
    for value in texts:
    ...
    for value in texts:
        email = pick_best_email(normalize_spoken_email(value or ""))
        if email:
            return email
...
async def demo_lead(request: Request):
    ...
    # HARD DEDUPE: Elfsight/Make can fire the same submission twice within seconds.
    # Block the duplicate BEFORE creating a second Retell call.
    ...
    if existing_cell:
        ...
        if seconds_old is not None and seconds_old < 180:
            print("DUPLICATE SUBMIT BLOCKED — recent submission already exists")
            return {
                "ok": True,
        ...
        if existing_status == "CALL_STARTED" and existing_call_id:
            print("DUPLICATE SUBMIT BLOCKED — active call already started")
            return {
                "ok": True,
                "skipped": "duplicate_active_call",
                "row": existing_row,
                "status": existing_status,
                "call_id": existing_call_id,
            }
